...
const newStr = value.replace(": ", "\":\"");
        $("#r_results").append("\""+newStr+"\",\n");
    });
  $("#r_results").append("}\n");
if(r_method == "POST"){
        $("#r_results").append("data = \""+r_formdata+"\"\n");
        $("#r_results").append("response = requests.post(url, data=data, headers=headers)\n");
    }else{
        $("#r_results").append("response = requests.get(url, headers=headers)\n");
    }
